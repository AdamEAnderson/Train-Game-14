...
(function () {
    ...
    var panZoomFunctions = {
        ...
        PanZoom = function (el, options) {
            ...
            settings.width = options.width || paper.width;
            settings.height = options.height || paper.height;
            settings.dragModifier = options.dragModifier || 1;
            ...
            function repaint() {
                ...
                var newWidth = settings.width * (1 - (me.currZoom * settings.zoomStep)),
                    newHeight = settings.height * (1 - (me.currZoom * settings.zoomStep));
    
                if (me.currPos.x < 0) {
                    me.currPos.x = 0;
                } else if (me.currPos.x > (settings.width * me.currZoom * settings.zoomStep)) {
                    me.currPos.x = (settings.width * me.currZoom * settings.zoomStep);
                }
    
                if (me.currPos.y < 0) {
                    me.currPos.y = 0;
                } else if (me.currPos.y > (settings.height * me.currZoom * settings.zoomStep)) {
                    me.currPos.y = (settings.height * me.currZoom * settings.zoomStep);
                }
            ...
            function dragging(e) {
                ...
                var evt = window.event || e,
                    newWidth = settings.width * (1 - (me.currZoom * settings.zoomStep)),
                    newHeight = settings.height * (1 - (me.currZoom * settings.zoomStep)),
                    newPoint = getRelativePosition(evt, container);
    
                deltaX = (newWidth * (newPoint.x - initialPos.x) / settings.width) * -1 * settings.dragModifier;
                deltaY = (newHeight * (newPoint.y - initialPos.y) / settings.height) * -1 * settings.dragModifier;
                initialPos = newPoint;
            ...
            function applyZoom(val, centerPoint) {
                ...
                if (me.currZoom < settings.minZoom) {
                    ...
                    centerPoint = centerPoint || { x: settings.width / 2, y: settings.height / 2 };
    
                    deltaX = ((settings.width * settings.zoomStep) * (centerPoint.x / settings.width)) * val;
                    deltaY = (settings.height * settings.zoomStep) * (centerPoint.y / settings.height) * val;
